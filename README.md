...
| 대여량 분포 분석 | 연도, 월, 자치구 조건에 따른 따릉이 대여량 히트맵 시각화 | `bike_usage_heatmap_cache` |
| 대여량 차트 분석 | 히트맵 캐시 데이터를 기반으로 대여소 Top5, 자치구 Top5, 월별 흐름 분석 | `bike_usage_heatmap_cache` |
...
기존 `예측 정확도 검증` 화면은 최종 웹 서비스 메뉴에서 제외하였다.  
대신 검증에 사용하던 모델 로딩 및 예측 계산 로직은 실시간 대여소 현황 화면에서 `1시간 뒤 예측 자전거 잔여 수` 컬럼을 계산하는 기능으로 재활용하였다.
```
